...
@section('title', __('Add New Fruit Product').' - Admin Matrif')
        ...
        <div class="w-full max-w-2xl bg-white rounded-2xl shadow-xl border border-slate-100 overflow-hidden">
            ...
            <div class="flex items-start justify-between px-6 py-5 bg-slate-50 border-b border-slate-100">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <h1 class="text-lg font-bold text-slate-800">{{ __('Add New Fruit Product') }}</h1>
            ...
            {{-- Form --}}
            <form method="POST" action="{{ route('admin.fruits.store') }}" enctype="multipart/form-data" class="px-6 py-5 space-y-5">
                @csrf
                    ...
                    <div>
                        ...
                        <div class="relative">
                            <input
                                ...
                                value="{{ $nextCode ?? 'FRUIT009' }}" readonly
                ...
                <div>
                    ...
                    >
                        ...
                        @foreach ($categories ?? ['Exotic', 'Bananas & Tropical', 'Citrus & Oranges', 'Apples & Pears', 'Berries & Melons'] as $i => $cat)
                            <option value="{{ is_array($cat) ? $cat['id'] : $i }}" {{ old('category_id') == (is_array($cat) ? $cat['id'] : $i) ? 'selected' : '' }}>
                ...
                <div>
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        ...
                        <label class="flex items-center justify-center gap-2 rounded-xl py-3.5 text-sm font-semibold cursor-pointer border-2 transition
                            ...
                            <input type="radio" name="unit" value="kg" class="hidden" checked>
                            <span class="leading-none">⚖️</span> Kilogram (kg)
                        </label>

                        <label class="flex items-center justify-center gap-2 rounded-xl py-3.5 text-sm font-semibold cursor-pointer border-2 transition
                                      border-slate-200 text-slate-600 bg-slate-50">
                            <input type="radio" name="unit" value="g" class="hidden">
                ...
                <div>
                    ...
                    <label for="fruit_image" class="flex items-center gap-4 bg-slate-50 border border-dashed border-slate-300 rounded-xl p-4 cursor-pointer hover:bg-slate-100 transition">
                        <img id="fruit-image-preview" src="" alt="" class="w-16 h-16 rounded-lg object-cover hidden">
                        <span id="fruit-image-placeholder" class="w-16 h-16 rounded-lg bg-slate-200 flex items-center justify-center text-2xl shrink-0">🥑</span>
            ...
            <div class="flex items-center justify-between gap-4 px-6 py-4 bg-slate-50 border-t border-slate-100">
                ...
                >
                    <span class="leading-none"><img src="{{ asset('icons/check_struk.png') }}" class="w-3 h-3 object-contain"></span> {{ __('Add Fruit') }}
                </button>
            </div>
