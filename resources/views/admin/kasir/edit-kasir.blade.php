...
@section('title', __('Edit Cashier').' - Admin Matrif')
...
@section('content')
    ...
    <div class="relative z-50 min-h-full flex items-start justify-center py-10 px-4">
        <div class="w-full max-w-xl bg-white rounded-2xl shadow-xl border border-slate-100 overflow-hidden">
            ...
            <div class="flex items-start justify-between px-6 py-5 border-b border-slate-100">
                <div class="flex items-center gap-3">
                    <span class="w-11 h-11 shrink-0 flex items-center justify-center rounded-xl bg-emerald-600 text-white text-lg">
                        <img src="{{ asset('icons/edit_kasir_form.png') }}" class="w-5 h-5 object-contain">
                    </span>
                    <div>
                        <h1 class="text-lg font-bold text-slate-800">{{ __('Edit Cashier') }}</h1>
                        <p class="text-sm text-slate-400">{{ __('Edit terminal credentials and assign store shift access.') }}</p>
                    </div>
                </div>
            ...
            {{-- Form --}}
            <form method="POST" action="{{ route('admin.kasir.update', $cashierId) }}" class="px-6 py-5 space-y-5">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <input
                        ...
                        value="{{ old('full_name', $cashier->name) }}"
                ...
                <div>
                    <label for="workstation" class="block text-sm font-semibold text-slate-700 mb-1.5">
                        {{ __('Assigned Workstation / Scale Register') }}
                    </label>
                    <div class="relative">
                        <span class="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 leading-none">🖥️</span>
                        <select
                            id="workstation" name="workstation_id"
                            class="w-full appearance-none pl-11 pr-10 py-3 rounded-xl bg-slate-50 border border-slate-200 text-sm font-medium text-slate-700 focus:outline-none focus:ring-2 focus:ring-emerald-500/30 focus:border-emerald-500"
                        >
                            <option value="floating" selected>{{ __('Floating / Flexible (Any Available Terminal)') }}</option>
                ...
                <div>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        ...
                        <label class="flex items-start gap-3 border border-slate-200 rounded-xl px-4 py-3.5 cursor-pointer">
                            <input type="radio" name="default_shift" value="shift_1" class="mt-1 text-emerald-600 focus:ring-emerald-500" {{ old('default_shift', $cashier->shift ?? 'shift_1') === 'shift_1' ? 'checked' : '' }}>
                            <span>
                        ...
                        <label class="flex items-start gap-3 border border-slate-200 rounded-xl px-4 py-3.5 cursor-pointer">
                            <input type="radio" name="default_shift" value="shift_2" class="mt-1 text-emerald-600 focus:ring-emerald-500" {{ old('default_shift', $cashier->shift) === 'shift_2' ? 'checked' : '' }}>
                            <span>
                ...
                <div class="flex items-center justify-end gap-4 pt-2 border-t border-slate-100 mt-6">
                    ...
                    >
                        <span class="leading-none"><img src="{{ asset('icons/fix_kasir_form.png') }}" class="w-5 h-5 object-contain"></span> {{ __('Edit Cashier Account') }}
                    </button>
                </div>
